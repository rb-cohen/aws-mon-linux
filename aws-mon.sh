...
if [ $LOAD_AVE1 -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "LoadAverage1Min" --value "$loadave1" --unit "Count" $CLOUDWATCH_OPTS 
    fi
fi
...
if [ $LOAD_AVE5 -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "LoadAverage5Min" --value "$loadave5" --unit "Count" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $LOAD_AVE15 -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "LoadAverage15Min" --value "$loadave15" --unit "Count" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $CONTEXT_SWITCH -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "ContextSwitch" --value "$context_switch" --unit "Count" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $INTERRUPT -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "Interrupt" --value "$interrupt" --unit "Count" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $CPU_US -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "CpuUser" --value "$cpu_us" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $CPU_SY -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "CpuUser" --value "$cpu_sy" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $CPU_ID -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "CpuIdle" --value "$cpu_id" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $CPU_WA -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "CpuWait" --value "$cpu_wa" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $CPU_ST -eq 1 ]; then
    ...
    if [ -n "$cpu_st" ]; then
        ...
        if [ $VERIFY -eq 0 ]; then
            aws cloudwatch put-metric-data --region $region --metric-name "CpuSteal" --value "$cpu_st" --unit "Percent" $CLOUDWATCH_OPTS
        fi
    fi
fi
...
if [ $MEM_UTIL -eq 1 -a $mem_total -gt 0 ]; then
    ...
    if [ $VERIFY -eq 0 -a -n "$mem_util" ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "MemoryUtilization" --value "$mem_util" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $MEM_USED -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "MemoryUsed" --value "$mem_used" --unit "$MEM_UNITS" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $MEM_AVAIL -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then        
        aws cloudwatch put-metric-data --region $region --metric-name "MemoryAvailable" --value "$mem_avail" --unit "$MEM_UNITS" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $SWAP_UTIL -eq 1 -a $swap_total -gt 0 ]; then
    ...
    if [ $VERIFY -eq 0 -a -n "$swap_util" ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "SwapUtilization" --value "$swap_util" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $SWAP_USED -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "SwapUsed" --value "$swap_used" --unit "$MEM_UNITS" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $SWAP_AVAIL -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "SwapAvailable" --value "$swap_avail" --unit "$MEM_UNITS" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $DISK_SPACE_UTIL -eq 1 -a -n "$DISK_PATH" -a $disk_total -gt 0 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "DiskSpaceUtilization$DISK_METRIC_SUFFIX" --value "$disk_util" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $DISK_SPACE_USED -eq 1 -a -n "$DISK_PATH" ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "DiskSpaceUsed$DISK_METRIC_SUFFIX" --value "$disk_used" --unit "$DISK_SPACE_UNITS" $CLOUDWATCH_OPTS
    fi
fi
...
if [ $DISK_SPACE_AVAIL -eq 1 -a -n "$DISK_PATH" ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --region $region --metric-name "DiskSpaceAvailable$DISK_METRIC_SUFFIX" --value "$disk_avail" --unit "$DISK_SPACE_UNITS" $CLOUDWATCH_OPTS
    fi
fi
